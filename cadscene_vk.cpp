...
void CadSceneVK::init(const CadScene& cadscene, VkDevice device, VkPhysicalDevice physicalDevice, VkQueue queue, uint32_t queueFamilyIndex, const Config& config)
{
  ...
  {
    // allocation phase
    m_geometryMem.init(device, physicalDevice, &m_memAllocator, config.needAddress, sizeof(CadScene::Vertex),
                       config.singleAllocation ? VkDeviceSize(4096) * MB : 256 * MB);
  ...
  m_buffers.matricesOrig =
      m_memAllocator.createBuffer(matricesSize, usageFlags | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, m_buffers.matricesOrigAID);
